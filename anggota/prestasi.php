...
// Ambil daftar prestasi berdasarkan ekstrakurikuler yang diikuti
$queryPrestasi = "SELECT prestasi FROM eskul WHERE id = $eskul_id";
$resultPrestasi = mysqli_query($conn, $queryPrestasi) or die("Error Query Prestasi: " . mysqli_error($conn));
$eskulPrestasi = mysqli_fetch_assoc($resultPrestasi);
$prestasiList = explode("\n", $eskulPrestasi['prestasi']); // Memisahkan prestasi menjadi array
?>
...
<head>
    ...
    <title>Prestasi | Monitoring Eskul</title>
...
<div class="content">
    ...
    <div class="container mt-4">
        <h2 class="text-center">Prestasi - <?php echo $eskul['nama']; ?></h2>
        <div class="row mt-3">
            <?php 
            if (!empty($eskulPrestasi['prestasi'])) {
                foreach ($prestasiList as $prestasi) { ?>
                    <div class="col-md-6 mb-4">
                        <div class="card p-3">
                            <h5 class="card-title"><i class="fas fa-medal"></i> <?php echo $prestasi; ?></h5>
                        </div>
                    </div>
                <?php }
            } else { ?>
                <p class="text-center text-muted">Belum ada prestasi yang tercatat.</p>
            <?php } ?>
        </div>
    </div>
</div>
